...
def prediction(ts_matrix, wa_matrices, phi, theta, prediction_lag=1):
    ...
    #prenp.reshape((len(phi),len(phi[0])))
    #pre_reshape=prenp.reshape((3,14))
...
'''
    ...
    prenp = np.array(pre)
    #
    for f in range(prediction_lag):
        for z in range(len(predictions[0])):
            predict=0
            t = len(predictions[0]) + f
            for tlag in range(len(phi)):
                for slag in range(len(phi[0])):
                #for slag in range(2):
                    predict += (wa_matrices[slag]).dot(predictions[(t - tlag),z] * phi[tlag, slag])
            for tlag in range(len(theta)):
                for slag in range(len(theta[0])):
                    predict -= (residuals[[t- tlag]] * theta[tlag, slag]).dot(wa_matrices[z].T)
                # TODO implement recursive innovation algorithm to update innovations/errors in prediction
                residuals = np.concatenate((residuals, np.zeros([1, len(residuals[0])])), axis=0)
            prenp=np.append(prenp,predict,axis=0)
    col=len(predictions[0])
    pre_reshape=prenp.reshape((prediction_lag,col))
    #pre_reshape = np.reshape((3,14))
    predictions = np.concatenate((predictions, pre_reshape), axis=0)
'''
